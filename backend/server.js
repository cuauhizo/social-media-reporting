...
app.get('/api/reports/:periodId', async (req, res) => {
  try {
    // 1. Encendemos los dos motores al mismo tiempo
    console.log('Iniciando fusión de datos...')
    const metricasExcel = await leerMetricasCSV()
    const hootsuiteData = await getSocialMetrics()
    const kpisManuales = await leerKpisGenerales()
    const kpisFbReales = await leerKpisFacebookHootsuite()

    // 2. LA FUSIÓN MÁGICA
    // Recorremos los 26 posts del Excel y les buscamos su foto en la API
    const postsFusionados = metricasExcel.map(postExcel => {
      // Por defecto, si la API no lo encuentra, le ponemos un cuadro gris
      let imagenMapeada = 'https://placehold.co/300x400/cccccc/ffffff?text=Post+Sin+Imagen'
      let tipoPost = 'POST'

      // Si la API nos trajo datos de Facebook, intentamos hacer el Match
      if (hootsuiteData && hootsuiteData.facebook && hootsuiteData.facebook.realPosts) {
        // Comparamos los primeros 20 caracteres del texto para saber si es el mismo post
        ...
        // Si lo encontramos en la API, le robamos la foto
        if (postCoincidente) {
      ...
      // Devolvemos el post armado como le gusta a tu Vue
      return {
        ...
        // Guardamos el texto por si lo quieres mostrar después
        text: postExcel.mensaje.substring(0, 60) + '...',
      }
    })

    // 3. ORDENAMOS Y CORTAMOS
    // Ordenamos de mayor a menor alcance (Los más virales primero)
    postsFusionados.sort((a, b) => b.reach - a.reach)

    // Si quieres mandar los 26, quítale el .slice(0, 8). Por ahora mandaremos los 8 mejores.
